...
import { createTemplateScenarios } from "@/lib/template-scenarios-db";
...
Template: ${campaign.name} (${campaign.advertiser}). Goal: ${campaign.goal || "general"}. Max CPM $${campaign.maxCPM}.
Brand keywords: ${campaign.brandKeywords.join(", ") || "none"}.`,
    ...
    const scenarios = await createTemplateScenarios(id, result.data.scenarios);
